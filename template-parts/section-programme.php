...
$pillars = mitsue_rows('pillars', [
  ...
  ['num'=>'iii', 'title_en'=>'Community Data Center', 'title_jp'=>'地域所有データセンター · Edge-scale', 'body'=>'The closed Mitsue Elementary School building, repurposed as a small-scale, energy-efficient edge-compute facility powered entirely by locally generated renewable energy.|Sized for accountability and community ownership — not hyperscale economics. Designed to be replicated by other depopulating municipalities.|By building a community-owned AI data center powered by locally generated renewable energy — solar and biomass-electric — Mitsue sets a working example of how rural communities can integrate technological progress with ecological sustainability. Not as opposites, but as a single coherent system. A model built to be copied.', 'body_jp'=>'廃校となった御杖小学校の校舎を、地域で発電した再生可能エネルギーのみで稼働する小規模・省エネ型エッジコンピュート施設として再活用します。|ハイパースケール経済ではなく、説明責任と地域所有のための規模。過疎に悩む他の自治体が複製できるモデルとして設計されています。|地域が所有し、太陽光とバイオマス発電という地元の再生可能エネルギーで稼働するAIデータセンターを構築することで、御杖は農村コミュニティが技術の進歩と生態系の持続可能性をどう統合できるかを示す実証モデルとなります。両者は対立するものではなく、一つの整合したシステムです。複製されることを前提に設計されたモデルです。', 'tag'=>'Edge compute · Heat re-use · Replicable · Progress + Sustainability'],
]);
?>
