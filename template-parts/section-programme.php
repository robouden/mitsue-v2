...
$pillars = mitsue_rows('pillars', [
  ...
  ['num'=>'iii', 'title_en'=>'Community Data Center', 'title_jp'=>'地域所有データセンター · Edge-scale', 'body'=>'The Mitsue Taiken Koryukan (体験交流館), repurposed as a small-scale, energy-efficient edge-compute facility powered entirely by locally generated renewable energy. Phase 2 expands to an old factory building in Mitsue.|Sized for accountability and community ownership — not hyperscale economics. Designed to be replicated by other depopulating municipalities.|By building a community-owned AI data center powered by locally generated renewable energy — biomass CHP and solar — Mitsue sets a working example of how rural communities can integrate technological progress with ecological sustainability. Not as opposites, but as a single coherent system. A model built to be copied.', 'body_jp'=>'御杖体験交流館を、地域で発電した再生可能エネルギーのみで稼働する小規模・省エネ型エッジコンピュート施設として活用します（フェーズ2では村内の旧工場建屋へ拡張）。|ハイパースケール経済ではなく、説明責任と地域所有のための規模。過疎に悩む他の自治体が複製できるモデルとして設計されています。|地域が所有し、バイオマスCHPと太陽光という地元の再生可能エネルギーで稼働するAIデータセンターを構築することで、御杖は農村コミュニティが技術の進歩と生態系の持続可能性をどう統合できるかを示す実証モデルとなります。両者は対立するものではなく、一つの整合したシステムです。複製されることを前提に設計されたモデルです。', 'tag'=>'Edge compute · Heat re-use · Replicable · Progress + Sustainability'],
]);
?>
